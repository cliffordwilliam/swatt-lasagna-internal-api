...
async function createOrder() {
    try {
        ...
        if (!response.ok) {
            ...
            console.log(`Order created successfully for ${payload.order_number}:`, data);
        }
...
async function runConcurrencyTest() {
    console.log("Running two concurrent create order requests with the same payload...");
    ...
    if (result1.status === 201 && result2.status === 201) {
        console.log("WARNING: Both requests succeeded. This might indicate a lack of proper concurrency handling, leading to duplicate orders if `order_number` is meant to be unique.");
    } else if (result1.status === 201 || result2.status === 201) {
        console.log("One request succeeded, the other failed (expected behavior if `order_number` is unique).");
    } else {
